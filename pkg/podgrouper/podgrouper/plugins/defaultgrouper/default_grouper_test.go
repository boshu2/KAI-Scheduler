...
func TestGetPodGroupMetadata_PodGroupNameLabel(t *testing.T) {
	trainPriorityClass := priorityClassObj(constants.TrainPriorityClass, 1000)
	kubeClient := fake.NewFakeClient(trainPriorityClass)
	...
	pod := &v1.Pod{
		ObjectMeta: v12.ObjectMeta{
			Namespace: "test_namespace",
			Labels: map[string]string{
				constants.PodGroupNameLabelKey: "my-custom-group",
			},
		},
	}
	...
	assert.Nil(t, err)
	assert.Equal(t, "my-custom-group", podGroupMetadata.Name)
}

func TestGetPodGroupMetadata_NoPodGroupNameLabel_AutoGenerated(t *testing.T) {
	trainPriorityClass := priorityClassObj(constants.TrainPriorityClass, 1000)
	kubeClient := fake.NewFakeClient(trainPriorityClass)
	...
	assert.Nil(t, err)
	expectedName := defaultGrouper.CalcPodGroupName(owner)
	assert.Equal(t, expectedName, podGroupMetadata.Name)
	assert.Contains(t, podGroupMetadata.Name, "pg-test_name-")
}

func TestGetPodGroupMetadata_EmptyPodGroupNameLabel_AutoGenerated(t *testing.T) {
	trainPriorityClass := priorityClassObj(constants.TrainPriorityClass, 1000)
	kubeClient := fake.NewFakeClient(trainPriorityClass)
	...
	pod := &v1.Pod{
		ObjectMeta: v12.ObjectMeta{
			Namespace: "test_namespace",
			Labels: map[string]string{
				constants.PodGroupNameLabelKey: "",
			},
		},
	}
...
func TestGetPodGroupMetadata_TwoPodsSharePodGroupName(t *testing.T) {
	trainPriorityClass := priorityClassObj(constants.TrainPriorityClass, 1000)
	kubeClient := fake.NewFakeClient(trainPriorityClass)

	owner1 := &unstructured.Unstructured{
		Object: map[string]interface{}{
			"kind":       "test_kind",
			"apiVersion": "test_version",
			"metadata": map[string]interface{}{
	...
	owner2 := &unstructured.Unstructured{
		Object: map[string]interface{}{
			"kind":       "test_kind",
			"apiVersion": "test_version",
			"metadata": map[string]interface{}{
	...
	sharedGroupName := "shared-training-group"
	...
	pod1 := &v1.Pod{
		ObjectMeta: v12.ObjectMeta{
			...
			Namespace: "test_namespace",
			Labels: map[string]string{
				constants.PodGroupNameLabelKey: sharedGroupName,
			},
		},
	}
	pod2 := &v1.Pod{
	...
	metadata1, err := defaultGrouper.GetPodGroupMetadata(owner1, pod1, convertOwnerToPartial(owner1))
	assert.Nil(t, err)

	metadata2, err := defaultGrouper.GetPodGroupMetadata(owner2, pod2, convertOwnerToPartial(owner2))
	assert.Nil(t, err)

	assert.Equal(t, sharedGroupName, metadata1.Name)
	assert.Equal(t, sharedGroupName, metadata2.Name)
	assert.Equal(t, metadata1.Name, metadata2.Name)
}
